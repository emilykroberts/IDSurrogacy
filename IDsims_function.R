...
tau_s = 1
tau_t = 2
frailtysd2 = frailtysd = 1 # inversely related to estimates of scale23 (becomes biased)
scenario = 2
...
scale13_1<-0.3 #Weibull scale for latent life time
...
if(T){ # problematic censoring if I calc Ttime_0 after this step
...
run_sim = function(SIM, rhos, rhot, frailtysd, params_list){
  ...
  if(T){
    ...
    dat0c = dat0[complete.cases(dat0),]
  ...
  #o23save0[1:(n/2)] = omega23_z0 = log(f0_23$frail)#*factor
  o23save0[!is.na(dat0$y23)] = omega23_z0[!is.na(dat0$y23)] = log(f0_23$frail)
  ...
  rgamma(1, shape = 0.01 + sum(dat1$s23[!is.na(dat1$y23)], na.rm = T),
              scale = (0.01 + 1/1 * sum(dat1$y23[!is.na(dat1$y23)] ^ 1 * exp(1 * omega23_z1), na.rm = T))^(-1))
  ...
  rgamma(1, shape = 0.01 + sum(dat0$s23[!is.na(dat0$s23)], na.rm = T),  scale = (0.01 + 1/1 * sum(dat0$y23[!is.na(dat0$s23)] ^ 1 * 
                           exp(1 * omega23_z0[!is.na(dat0$s23)]), na.rm = T))^(-1))
...
while(z < SIM){ 
  ...
  for(i in 1:(n/2)){
    ...
    p[i]  = exp( sap[i] - sap2[i] +
                   log( (pnorm(omega12_star[i], 0, sd = frailtysd2))) - 
                   log( (pnorm(omega12_z0[i], 0, sd = frailtysd2)) ) )
    
    if(!individual){  p[i]  = exp( sap[i] - sap2[i] +
                                     sum( log(pnorm(omega12_star, 0, sd = frailtysd2)))-
                                     sum(log(pnorm(omega12_z0, 0, sd = frailtysd2)) ))}
  ...
  shape12prior_c = ptruncnorm(holdshape12_0[z-1], a = 0, b = 3,  mean = 0, sd = 3)
    ...
    p = exp(like12_shape(shape12_0 = shape12_0_star, scale12_0 = scale12_0_star, omega12_z0 = omega12_z0,
            dat0 = dat0, c12_0 = c12_0_star) + log(shape12prior_p) - #like12(c12_0_star) + log(c12prior_p) +
            ...
            #  (like12(holdc12_0[z-1])+ log(c12prior_c))
            )
  ...
  for(i in 1:(n/2)){
    ...
    p[i]  = exp( sap[i] - sap2[i] +
                   log( (pnorm(omega13_star[i], 0, sd = frailtysd2))) - 
            
                          log( (pnorm(omega13_z0[i], 0, sd = frailtysd2)) ) )
    
    if(!individual){  p[i]  = exp( sap[i] - sap2[i] +
                   sum( log(pnorm(omega13_star, 0, sd = frailtysd2)))-
                   sum(log(pnorm(omega13_z0, 0, sd = frailtysd2)) ))}
  ...
  if(holdshape13) shape13_0_star = 1
  ...
  if(holdshape23) shape23_0_star = 1
...
for(i in 1:(n/2)){
  ...
  p[i]  = exp( sap[i] - sap2[i] +
                 log( (pnorm(omega12_star[i], 0, sd = frailtysd2))) - 
                 log( (pnorm(omega12_z1[i], 0, sd = frailtysd2)) ) )
  
  if(!individual){  p[i]  = exp( sap[i] - sap2[i] +
                 sum( log(pnorm(omega12_star, 0, sd = frailtysd2)))-
                 sum(log(pnorm(omega12_z1, 0, sd = frailtysd2)) ))}
  ...
  for(i in 1:(n/2)){
    ...
    p[i]  = exp( sap[i] - sap2[i] +
                   log( (pnorm(omega13_star[i], 0, sd = frailtysd2))) - 
                   log( (pnorm(omega13_z1[i], 0, sd = frailtysd2)) ) )
    
    if(!individual){  p[i]  = exp( sap[i] - sap2[i] +
                   sum( log(pnorm(omega13_star, 0, sd = frailtysd2)))-
                   sum(log(pnorm(omega13_z1, 0, sd = frailtysd2)) ))
    }
  ...
  if(holdshape13) shape13_1_star = 1
  ...
  if(holdshape23) shape23_1_star = 1
