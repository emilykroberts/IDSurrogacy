...
setwd("./results/supplement/TableS2_1_3") # save supplemental simulations results in TableS2_1_3 folder

# corresponds to Supplement Table 2 tableS2results_1_3.csv (Supplemental Table 2 rows 1-3) by setting equalfrail = F and independent = T
# holdshape and holdtheta within run_sim.R is set to T for simplicity of this sensitivity analysis
# simulate data
set.seed(1 + array_id)
...
dat = sim_data(n = n, array_id = array_id, scenario = scenario, effecttheta = effecttheta, 
               ...
               diffscale1323 = diffscale1323, specify = specify, equalfrail = F, independent = T)
...
# run simulation and write final results
set.seed(1 + array_id)
params_res = run_sim(SIM = SIM, array_id = array_id, rhos = rhos, rhot = rhot, frailtysd = frailtysd, 
                     ...
                     proposalsdfrail = proposalsdfrail, proposalsd = proposalsd, equalfrail = F, independent = T, 
                     MFS = F, holdshape = T, holdtheta = T
)

final_results(params_matrix = params_res, write = write)
# note: figure in manuscript corresponds to array_id = 12
if(array_id == 12){
  plot_results(params_matrix = params_res, write = plotwrite, fignum = 'S5') # creates CEP file FigureS5.jpeg
}