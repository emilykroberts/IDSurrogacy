...
run_sim = function(SIM, rhos, rhot, frailtysd, params_list){
  ...
  if(T){
    ...
    penalfrail1 = frailtyPenal(Surv(time, status)~cluster(id),  data=dat1penal, hazard = "Weibull",
                              RandDist = "LogN",  init.Theta = frailtysd)
    
    omega12_z1 = o12save1[1:(n/2)] = exp(penalfrail1$frailty.pred)
    
    penalfrail1 = frailtyPenal(Surv(time, status)~cluster(id),  data=dat1penal, hazard = "Weibull",
                              init.Theta = frailtysd)
    
    dat1penal = data.frame(time = c(dat1$y12, dat1$y13),  status = c(dat1$s12, dat1$s13),
                           y12 = c(dat1$y12, rep(0, n/2)),
                           id = rep(1:(n/2), 2), death = c( dat1$s13, rep(0, n/2)))
    ...
    penalfrail1 = frailtyPenal(Surv(time, status)~ cluster(id) + terminal(death),
                           jointGeneral = TRUE,
                               data=dat1penal, n.knots = 4, kappa = c(1))
    ...
    penalfrail1 = frailtyPenal(Surv(time, status)~cluster(id),
                               ...
                               RandDist = "LogN",  init.Theta = frailtysd)
    # 
    # fit <- fitfrail(Surv(time, status) ~ y12 + cluster(id),
    #                 dat1penal, frailty="lognormal")
    # 
    
    t23 = rowSums(cbind(dat1$y23, dat1$y12), na.rm = F)
    dat1penal = data.frame(time = c(t23, dat1$y12),  status = c(dat1$s23, rep(0, n/2)),
                           y12 = c(dat1$y12, rep(0, n/2)), 
                           id = rep(1:(n/2), 2), death = c( dat1$s13, dat1$s13))
    
    dat1penal = dat1penal[complete.cases(dat1penal),]
    
    # dat1penal = data.frame(time = c(t23, dat1$y13),  status = c(dat1$s23, dat1$s13),
    #                        y12 = c(dat1$y12, rep(0, n/2)), 
    #                        id = rep(1:(n/2), 2), death = c( dat1$s13, rep(0, n/2)))
    # 
    # dat1penal = dat1penal[complete.cases(dat1penal),]
     ...
     #                            data=dat1penal, n.knots = 4, kappa = c(1))
     # 
     # dat1penal = data.frame(time = ret$y2 - ret$y1)
     # dat1penal$status = ret$delta2
     # dat1penal$ID = as.numeric(1:nrow(dat1penal))

    omega23_z1 = o23save1[1:(n/2)] = omega13_z1 = o13save1[1:(n/2)] = (penalfrail1$frailty.pred)
    #omega13_z1[dat1$s23 == 1] = o13save1[dat1$s23 == 1] = rnorm(sum(dat1$s23 == 1), sd = frailtysd)
    
    plot(log(f1_23$frail), omega23_z1, col = dat1$s23 + 1)
    plot(log(f1$frail), omega12_z1, col = dat1$s12 + 1)
    
    rgamma(1, shape = 0.01 + sum(dat1$s23, na.rm = T),
           scale = (0.01 + 1/1 * sum(dat1$y23^ 1 * exp(1 * penalfrail1$frailty.pred), na.rm = T))^(-1))
    
    dat1penal = data.frame(time = c(dat1$y23),  status = c(dat1$s23), y12 = c(dat1$y12), 
                           id = rep(1:(n/2), 1))
    dat1penal = dat1penal[complete.cases(dat1penal),]
    ...
    #omega23_z1 = o23save1[1:(n/2)] = omega13_z1 = o13save1[1:(n/2)] = rnorm(n/2, sd = frailtysd)
    ...
    penalfrail0 = frailtyPenal(Surv(time, status)~ cluster(id) + terminal(death),
                               jointGeneral = TRUE,
                               data=dat0penal, n.knots = 4, kappa = c(1))
   ...
   # dat0penal = data.frame(time = c(dat0$y13),status = c(dat0$s13),   id = rep(1:(n/2), 1))
    ...
    #omega13_z0[dat0$s23 == 1] = o13save0[dat0$s23 == 1] = rnorm(sum(dat0$s23 == 1), sd = frailtysd)
  ...
  mean(rgamma(100,  shape = 0.01 + sum(dat1$s13, na.rm = T),
         scale = (0.01 + 1/1 * sum(dat1$y13^ 1 * exp(1 * omega13_z1), na.rm = T))^(-1)))
  
  mean(rgamma(100,  shape = 0.01 + sum(dat1$s12, na.rm = T),
         scale = (0.01 + 1/1 *sum(dat1$y12 ^ 1 * exp(1 * omega12_z1), na.rm = T))^(-1)))

  mean(rgamma(100,  shape = 0.01 + sum(dat0$y23, na.rm = T),
         scale = (0.01 + 1/1 * sum(dat0$s23^ 1 * exp(omega13_z0), na.rm = T))^(-1)))
  
  mean(rgamma(100,  shape = 0.01 + sum(dat0$s13, na.rm = T),
         scale = (0.01 + 1/1 *sum(dat0$y13 ^ 1 * exp(1 * omega13_z0), na.rm = T))^(-1)))

 mean(rgamma(100, shape = 0.01 + sum(dat0$s12, na.rm = T),
         scale = (0.01 + 1/1 *sum(dat0$y12 ^ 1 * exp(1 * omega12_z0), na.rm = T))^(-1)))
...
if(holdfrail13)
  ...
  if(F){
#  omega12_z0 = o12save0[1:(n/2)] = rnorm(n/2, o12save0[1:(n/2)], sd = frailtysd)
  omega13_z0 = o13save0[1:(n/2)] = rnorm(n/2, o13save0[1:(n/2)], sd = frailtysd)

#  omega12_z1 = o12save1[1:(n/2)] = rnorm(n/2, o12save1[1:(n/2)], sd = frailtysd)
  omega13_z1 = o13save1[1:(n/2)] = rnorm(n/2, o13save1[1:(n/2)], sd = frailtysd)
  }
...
if(F){
  scale23_0_star = rgamma(1, shape = 0.01 + sum(dat0$s23[!is.na(dat0$y23)], na.rm = T),
                          scale = (0.01 + 1/shape23_0_star *
                                     sum(dat0$y23[!is.na(dat0$y23)] ^ shape23_0_star * 
                                           exp(c23_0_star * omega23_z0[!is.na(dat0$y23)] +
                                                 theta23_0_star * dat0$y12[!is.na(dat0$y23)]), na.rm = T))^(-1))
  ...
  theta23prop_c = pnorm(holdtheta23_0[z-1], 0, sd = proposalsd)
    ...
    p = exp(like13_shape(dat0 = dat0, c13_0 = c13_0_star,
    ...
    # p = exp(like13_shape(dat0 = dat0, c13_0 = c13_0_star,
    #                      shape13_0 = shape13_0_star, scale13_0 = holdscale13_0[z-1], omega13_z0 = omega13_z0) +
    #           log(shape13prior_p)-(like13_shape(dat0 = dat0, c13_0 = holdc13_0[z-1],
    #                         shape13_0 = holdshape13_0[z-1], scale13_0 = holdscale13_0[z-1], omega13_z0 = omega13_z0)+ log(shape13prior_c)) )
    # 
    
    p2 = exp(   like23_theta(theta23 = theta23_0_star, dat0 = dat0,c23_0 = holdc23_0[z-1], omega13_z0 = omega13_z0,
                             shape23_0 = holdshape23_0[z-1], scale23_0 = holdscale23_0[z-1]) + 
...
if(F){
    p2 = exp(   like23_theta(theta23 = theta23_0_star, dat0 = dat0,c23_0 = holdc23_0[z-1], omega13_z0 = omega13_z0,
                             shape23_0 = holdshape23_0[z-1], scale23_0 = scale23_0_star) + 
                  log(theta23prior_p) + log(theta23prop_p) -
                  (like23_theta(theta23_0 = holdtheta23_0[z-1],dat0 = dat0, omega13_z0 = omega13_z0, c23_0 = holdc23_0[z-1],
                                shape23_0 = holdshape23_0[z-1], scale23_0 = scale23_0_star)+ log(theta23prior_c) + log(theta23prop_c))
                + like23_shape(shape23_0 = shape23_0_star, dat0 = dat0, omega13_z0 = omega13_z0, c23_0 = holdc23_0[z-1],
                               scale23_0 = scale23_0_star, theta23_0 = holdtheta23_0[z-1]) + log(shape23prior_p)-
                  (like23_shape(shape23_0 = holdshape23_0[z-1], dat0 = dat0, c23_0 = holdc23_0[z-1], omega13_z0 = omega13_z0, 
                                scale23_0 = scale23_0_star, theta23_0 = holdtheta23_0[z-1]) + log(shape23prior_c)))
...
if(F){
  scale23_1_star = rgamma(1, shape = 0.01 + sum(dat1$s23[!is.na(dat1$y23)], na.rm = T),
                          scale = (0.01 + 1/shape23_1_star *
                                     sum(dat1$y23[!is.na(dat1$y23)] ^ shape23_1_star * 
                                           exp(c23_1_star * omega23_z1[!is.na(dat1$y23)] +
                                                 theta23_1_star * dat1$y12[!is.na(dat1$y23)]), na.rm = T))^(-1))
...
#print(p2)
    ...
    # p2 = exp(   like23_theta1(theta23 = theta23_1_star, dat1 = dat1,c23_1 = holdc23_1[z-1], omega13_z1 = omega13_z1,
    #                           shape23_1 = holdshape23_1[z-1], scale23_1 = scale23_1_star)+
    #               log(theta23prior_p) + log(theta23prop_p) -
    #               (like23_theta1(theta23_1 = holdtheta23_1[z-1],dat1 = dat1, omega13_z1 = omega13_z1, c23_1 = holdc23_1[z-1],
    #                              shape23_1 = holdshape23_1[z-1], scale23_1 = holdscale23_1[z-1])+ log(theta23prior_c) + log(theta23prop_c))
    #             + like23_shape1(shape23_1 = shape23_1_star, dat1 = dat1, omega13_z1 = omega13_z1, c23_1 = holdc23_1[z-1],
    #                             scale23_1 = scale23_1_star, theta23_1 = holdtheta23_1[z-1]) + log(shape23prior_p)-
    #               (like23_shape1(shape23_1 = holdshape23_1[z-1], dat1 = dat1, c23_1 = holdc23_1[z-1], omega13_z1 = omega13_z1,
    #                              scale23_1 = holdscale23_1[z-1], theta23_1 = holdtheta23_1[z-1]) + log(shape23prior_c)))
    # 
